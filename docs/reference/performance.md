...
These timings apply to the bundled assembly and the run configuration above.
Runtime and memory depend on input size, hit count, storage, and available CPUs.
The source measurements are in
[`example_performance.tsv`](../data/example_performance.tsv). The figure is
generated by `notebooks/example_performance.ipynb`.
