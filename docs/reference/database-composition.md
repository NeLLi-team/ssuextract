...
Database release 1.0.0 is archived at
[Zenodo](https://doi.org/10.5281/zenodo.21367799). The `curated` profile contains
SILVA 138.2 and PR2 5.1.1. The `img` profile contains the same references plus
IMG 16S rRNA gene and 18S rRNA gene sequences from eukcensus 2025.

## Profiles

| Profile | Source records | Unique sequences | 16S rRNA gene index | 18S rRNA gene index | Download size |
| --- | ---: | ---: | ---: | ---: | ---: |
| `curated` | 683,597 | 609,298 | 416,021 | 193,282 | 345.5 MiB |
| `img` | 1,680,777 | 1,396,949 | 1,067,340 | 329,614 | 828.9 MiB |
...
Eukaryota. In release 1.0.0, IMG sequences with a preferred domain assignment
...
Terminal PR2 compartment suffixes (`:apic`, `:chro`, `:mito`, `:nucl`, and
`:plas`) are counted with their parent supergroup.
...
*Figure 2. Named lineage distributions in the SILVA and PR2 reference
component. Percentages use sequences resolved to the reported rank as the
denominator. Less abundant named lineages are combined as “Other named.”*

## IMG sampling coordinates

The released IMG metadata retains the IMG Taxon OID and latitude/longitude when
...
describe the released metadata; repeated taxa or studies at one coordinate do
...
- [Named lineage counts](../data/database_taxonomy_lineages.tsv)
