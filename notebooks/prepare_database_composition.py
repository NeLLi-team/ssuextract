...
PR2_COMPARTMENT_SUFFIXES = ("apic", "chro", "mito", "nucl", "plas")
...
def taxonomy_rank_rows(
    ...
    if observed_suffixes != set(PR2_COMPARTMENT_SUFFIXES):
        raise CompositionError(
            f"unexpected PR2 supergroup suffixes for {profile}: {sorted(observed_suffixes)}"
        )
    connection.execute(
        """CREATE OR REPLACE TEMP TABLE rank_assignments AS
           ...
           SELECT domain,
                  CASE
                    WHEN domain = 'Eukaryota' THEN regexp_replace(
                        raw_lineage, ':(apic|chro|mito|nucl|plas)$', ''
                    )
                    ELSE raw_lineage
                  END AS lineage
           FROM parsed""",
...
def main() -> None:
    ...
    taxonomy_summary["Eukaryota"]["normalization"] = (
        "terminal PR2 compartment suffixes are collapsed to the parent supergroup"
    )
    taxonomy_summary["Eukaryota"]["collapsed_suffixes"] = list(PR2_COMPARTMENT_SUFFIXES)
    taxonomy_summary["unresolved_definition"] = (
        "no non-empty second taxonomy segment, or a second segment labeled unclassified or incertae sedis"
    )
    outputs = [
        composition_path,
        sources_path,
    ...
    provenance_output = {
        ...
        "database_release": {
            ...
            "zenodo_record_id": catalog["zenodo"]["record_id"],
            "zenodo_concept_record_id": catalog["zenodo"]["concept_record_id"],
            "doi": f"10.5281/zenodo.{catalog['zenodo']['record_id']}",
            "url": f"https://zenodo.org/records/{catalog['zenodo']['record_id']}",
        },
        "build_source_tree": {
